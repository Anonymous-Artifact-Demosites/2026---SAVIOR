"""URL pre-filtering for SAVIOR (§6.3).
...
This module extracts the filtering logic from the original
oauth_test_runner.py's test_single_url() function.
"""
...
def format_url(url_input: str) -> str | None:
    ...
    Exact replication of oauth_test_runner.py::format_url():
    ...
    # CSV: take LAST column (matches original)
    if "," in url:
        parts = url.split(",", 1)
        url = parts[-1].strip()
...
def read_url_list(urls=None, url_file=None) -> list[str]:
    ...
    Exact replication of oauth_test_runner.py::read_url_list(),
    including stdin pipeline support.
    """
    result = []
    ...
    # From stdin (pipeline) — matches original
