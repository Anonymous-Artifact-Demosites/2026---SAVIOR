"""Shared 2captcha integration used by compatibility wrappers and new modules."""
...
def _captcha_script_path():
    """Return the repository-local 2captcha.py path, supporting both layouts."""
    base = Path(__file__).resolve()
    bundled = base.parents[1] / "2captcha.py"
    if bundled.exists():
        return bundled
    return base.parents[2] / "2captcha.py"


def _run_python_mode(args):
    """Execute 2captcha.py via subprocess in normal Python mode."""
...
def _run_packaged_mode(args):
    """Execute the bundled 2captcha module when running from a packaged executable."""
    original_argv = sys.argv.copy()
    sys.argv = ["2captcha"] + list(args)
    try:
        ...
        return {
            ...
            "stderr": json.dumps(
                {
                    "success": False,
                    "error": f"Exception in 2captcha: {str(exc)}",
                }
            ),
        }
    finally:
...
def run_args(args, packaged=None):
    """Run 2captcha with the provided argument vector and return stdout/stderr."""
    if packaged is None:
...
def solve(captcha_type, site_key=None, url=None, extra_args=None, packaged=None):
    """Solve a captcha using the shared 2captcha.py subprocess entry point."""
    args = [captcha_type]
...
def call_2captcha_entry(argv=None):
    """CLI-compatible implementation of call_2captcha.py."""
...
def captcha_wrapper_call(args):
    """Function-compatible implementation of captcha_wrapper.call_2captcha."""
    result = run_args(args)
    if result["stdout"]:
        print(result["stdout"], end="")
    if result["stderr"]:
...
def captcha_wrapper_entry(argv=None):
    ...
    args = list(sys.argv[1:] if argv is None else argv)
    if not args:
        print("Usage: python captcha_wrapper.py <2captcha_args...>")
