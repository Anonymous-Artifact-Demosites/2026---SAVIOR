"""Rule-based invariant evaluation for SAVIOR tasks (§5.2.3).
...
def _configs_path() -> Path:
    ...
    if bundled.exists():
        return bundled
    # Original layout: repo/savior/state_auditor/invariants.py → repo/configs/
...
def get_task_config(task_id: str) -> dict:
    ...
    if normalized not in configs:
        raise NotImplementedError(f"Invariants for {task_id} are not defined in invariants.yaml")
    ...
    # Generate regex pattern for legacy STATUS extraction
...
def _evaluate_leaf(condition: dict, observations: dict | None) -> bool:
...
def _evaluate_condition(condition: dict, observations: dict | None) -> bool:
    """Recursively evaluate a structured condition tree against observations."""
    ctype = condition.get("type")
    if ctype == "leaf":
        return _evaluate_leaf(condition, observations)
    if ctype == "all_of":
        return all(_evaluate_condition(c, observations) for c in condition.get("conditions", []))
    if ctype == "any_of":
        return any(_evaluate_condition(c, observations) for c in condition.get("conditions", []))
    if ctype == "not":
        return not _evaluate_condition(condition["condition"], observations)
...
def _evaluate_verdict_rules(verdict_rules: list, invariant_results: dict, observations: dict) -> dict | None:
    """Walk a verdict_rule list top-to-bottom, return first match."""
    for rule in verdict_rules:
        ...
        if "invariant" in condition:
            ...
            actual_violated = invariant_results.get(inv_id, False)
        ...
        if "type" in condition:
            if _evaluate_condition(condition, observations):
                return {"status": rule["status"], "reason": rule.get("reason", "")}
            continue
...
def evaluate_invariants(task_id: str, observations: dict | None) -> dict | None:
    ...
    Implements the rule engine described in REFACTORING_PLAN_v2 section 4.
