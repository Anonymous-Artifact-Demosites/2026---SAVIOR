"""Batch URL scanning runner for SAVIOR (§6 Real-World Evaluation).
...
The original oauth_test_runner.py is refactored into this module.
The root-level oauth_test_runner.py becomes a thin compatibility wrapper.
"""
...
# ---------------------------------------------------------------------------
# OAuth support scanning (Phase 1) — full replication of
# oauth_test_runner.py::test_single_url() lines 96-301
# ---------------------------------------------------------------------------
...
def scan_oauth_support(url: str) -> dict:
    ...
    Full replication of oauth_test_runner.py::test_single_url().
    ...
    # Build scanning prompt (verbatim from oauth_test_runner.py lines 139-188)
...
def _find_available_scripts() -> list[dict]:
    ...
    if bundled.exists():
        ...
        # Original layout: repo/savior/runner/ → repo/tasks/
...
def select_test_script() -> dict | None:
    ...
    Preserves the original oauth_test_runner menu format.
    """
...
def invoke_test_script(script_path: str, target_urls: list[str], *, api_key: str = "", iterations: int = 1) -> dict:
    ...
    counted as success if at least one iteration succeeds (§6.2
    ...
    for index, url in enumerate(target_urls, 1):
        ...
        env = os.environ.copy()
        if api_key:
            env["TWOCAPTCHA_API_KEY"] = api_key
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help="Repeat each task N times per URL to observe execution stability "
             ...
             "For structured verdict aggregation per §6.2, use "
    ...
    print()
    print("-" * 60)
    print("  2Captcha API Key Configuration")
    print("-" * 60)
    print("  Some test scripts may encounter CAPTCHAs during execution.")
    print("  Please enter your 2Captcha API Key (or press Enter to skip)")
    print("  Get API key from: https://2captcha.com")

    try:
        api_key = input("2Captcha API Key: ").strip()
    except (EOFError, KeyboardInterrupt):
        api_key = ""
    ...
    if api_key:
        ...
        print("  No API key provided. CAPTCHA solving may not work.")
    ...
    # Execute
    result = invoke_test_script(script["path"], target_urls, api_key=api_key, iterations=args.iterations)
